...
static Array<int,9> create_array_of_bytes(float coord){
    float primo;
    Array<int, 9> bytes;
    primo = coord / 100;
    bytes[0] = int(primo);
    for (int i = 1; i < 9; i++) {
        primo = primo * 10 - int(primo) * 10;
        bytes[i] = int(primo);
    }
    return bytes;
}
...
void printGPSInfo()
{
  ...
  // and the number of visible satellites.   //float a 32 bit //44.881220----> voglio un array da 9 posti
  //SerialMonitor.println(tinyGPS.location.lat(), 6);
  //SerialMonitor.println(tinyGPS.location.lng(), 6);
  //SerialMonitor.println();
  SerialMonitor.write(0xff);
  SerialMonitor.write(0x13);
  SerialMonitor.write(0x01);
  Array<int,9> lat;
  lat=create_array_of_bytes(44.881220);//sta da mettere tinyGPS.location.lat() 
  Array<int,9> lng;
  lng=create_array_of_bytes(11.067926);//sta da mettere tinyGPS.location.lng()
  for(int i=0; i<9; i++){
    SerialMonitor.write(lat[i]);
  }
  for(int i=0; i<9; i++){
    SerialMonitor.write(lng[i]);
  }
  SerialMonitor.write(0xfe);
}
